...
class DistanceCalc
  attr_reader :line_1, :line_2
  MOVE_MAP = {
    'R' => ->(pos) {  x = pos.dup; x[0] += 1; x },
    'L' => ->(pos) {  x = pos.dup; x[0] -= 1; x },
    'U' => ->(pos) {  y = pos.dup; y[1] += 1; y },
    'D' => ->(pos) {  y = pos.dup; y[1] -= 1; y }
  }
  def initialize(*args)
    @position = [0, 0]
    args.each_with_index do |line, index|
      draw(line, index)
      @position = [0, 0]
    end
  end
  ...
  def find_smallest_intersection
    intersections[1..-1].map { |pair| pair[0].abs + pair[1].abs }.min
  end
  ...
  def find_steps_taken
    intersections.map do |intersection|
      line_2.index(intersection) + line_1.index(intersection)
    end.sort[1..-1].min
  end
  ...
  def draw(line, index)
    ...
    line.map do |moves|
      transform_to_coords(moves).each do |pos|
        instance_variable_get(var) << pos
      end
    end
  end

  def transform_to_coords(moves)
    direction, times = moves[0], moves[1..-1]
    positions = []
    start = @position.dup
    times.to_i.times do
      positions << start
      start = MOVE_MAP[direction].call(start)
    end
